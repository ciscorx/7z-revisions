;; 7z-revisions.el            -*- lexical-binding: t; -*-          
...
;; what you may have accidentally deleted from your source code or
;; document?  7z-revisions is now available, an indispensable emacs
;; org-mode companion that does exactly that.  With the
...
;; patch to the archive, representing a diff between revisions.  And,
;; optionally, the .7z extension can be altered to something else,
;; such as .8z, by setting the global variable
...
;; past revisions in the archive, providing word by word differential
...
;; document, or they will not work.
;; 
;; Here are some useful commands that can be used editing your document (most of these can be found in the menu options):
;;     M-x 7zr-line-last-changed-on
;;     M-x 7zr-goto-line-of-last-revision
;;     M-x 7zr-goto-rev-region-last-changed
...
;;     M-x 7zr-rename-document-and-its-archive
;;     M-x 7zr-regenerate-all-hashes-from-diffs
;;     M-x 7zr-refresh-all-bookmarks
...
;;   date, h = toggle highlight differences, H = highlight revisions
;;   that have notes associated with them and color code them if
;;   notes include colors spelled out in all caps, c = edit revision
;;   note, w = display day of the week
;;
...
;;   diff file, w = display day of the week, F2 C-p view to the past
;;   revision that changes the selected region, F2 C-F2 = view
;;   revision of where lines in an active region have last been
;;   modified, g = Quit 7z-revisions and then try to goto the line in
...
;;
;; In dired-mode, the key binding C-c z = 7zr-dired-7z-revisions,
;;   which views the 7z-revisions archive at point.
;;
;; Required features:
...
;; enable 7z-revisions-mode for org-mode, text-mode, lua-mode and emacs-lisp-mode, making them automatically update/create 7z-revisions archives on each save
...
(setq 7zr-temp-directory-windows-nt temporary-file-directory)   ; temp directory to use on windows machines
(setq 7zr-temp-directory-linux temporary-file-directory)
(setq 7zr-temp-directory-apple temporary-file-directory)
...
(setq 7zr-summary-view-revision_last_rev "")
...
(setq 7zr-summary-last-line-last 0)
...
; (setq 7zr-hasht (make-hash-table :size 20000 :test 'equal))  ; this must instead be a buffer local variable
...
(setq 7zr-revisions_tmpbuffer-default_prefix_name "7zr-revisions_of_")
...
(setq 7zr-goto-rev-region-last-changed_linenum_begin 0)
(setq 7zr-goto-rev-region-last-changed_lines 0) 
...
(setq 7zr-os_is_windows_p (string-match-p "windows" 7zr-os_type))
...
(setq 7zr-preserved-evil-emacs-state-p nil)
...
(setq 7zr-windows-nt nil)
(setq 7zr-mswindows-requirements-failed nil)
(cond
 ((string-equal system-type "windows-nt") ; Microsoft Windows
  (setq 7zr-windows-nt t)
  (setq 7zr-temp-directory 7zr-temp-directory-windows-nt)
  (cond
	((not (executable-find "diff.exe"))
	 (setq 7zr-mswindows-requirements-failed "Need diff.exe from http://gnuwin32.sourceforge.net/packages/diffutils.htm or http://msys2.org.  If msys2 you may need to issue pacman -S diffutils from the msys2 terminal"))
...
;; HOOKS----------------------------------------
;; add hook to dired mode to view 7z-revisions archives using the C-c z command
(add-hook 'dired-mode-hook (lambda () (define-key dired-mode-map (kbd "C-c z")  (lambda () (interactive) (7zr-dired-7z-revisions)))))

					;or run from dired-guess-shell-alist-user
...
(setq 7zr-summary-mode-map
  (let ((map (make-sparse-keymap)))
    ...
    (define-key map (kbd "H") '7zr-summary-highlight-all-revs-with-notes)
    ...
    (define-key map [menu-bar 7zr-summary highlight-all-notes]
      '(menu-item "Highlight notes" 7zr-summary-highlight-all-revs-with-notes
                  :help "Highlight all revs that have revision notes."))
...
(define-derived-mode 7zr-summary-mode nil "7zr-sum"
  "Major mode for summary of a 7z archive.
...
;    (define-key map (kbd "<f2> C-o") '7zr-order-raw-notes-file)    
    ...
    (define-key map (kbd "<f2> C-<f2>") '7zr-regenerate-all-hashes-from-diffs)
    ...
    (define-key map [menu-bar 7z-revisions-mode regenerate-hashes]
      '(menu-item "Regenerate all hashes from diffs" 7zr-regenerate-all-hashes-from-diffs
		  :help "Regenerate all sha1 hash values from diffs and compare last hash with latest revision."))
    ...
    (define-key map [menu-bar 7z-revisions-mode rev_last_changed2]
      '(menu-item "Goto rev that changed region" 7zr-goto-rev-region-last-changed
                  :help "Goto the past revision that changed selected region."))
...
(setq 7zr-minibuffer-input-mode-map 
  (let ((map (make-sparse-keymap)))
    (define-key map (kbd "C-c C-c") 'exit-minibuffer)
  ;  (define-key map (kbd "C-c C-k") 'exit-minibuffer)
...
(setq 7zr-metafile-mode-map 
  (let ((map (make-sparse-keymap)))
    (define-key map (kbd "C-c C-c") '7zr-metafile-exit)
    (define-key map (kbd "C-c C-k") '7zr-metafile-cancel)
    (define-key map (kbd "<f2> s") '7zr-select-tag-to-insert-into-document)
    (define-key map [menu-bar 7zr-metafile]
      (cons "7zr-metafile" (make-sparse-keymap "7zr-metafile")))
    (define-key map [menu-bar 7zr-metafile select-tag]
      '(menu-item "Insert Tag" 7zr-select-tag-to-insert-into-document
                  :help "Select a tag to insert"))
...
Turning on 7zr-metafile-mode runs the normal hook `7zr-metafile-mode-hook'."
  ...
  (message "Press C-c C-c to save and return to last buffer, or C-c C-k to cancel, when finished editing.  You will need to disable read-only on buffer before editing.")
 ; (hl-line-mode t)
)
...
(defun 7zr-line-number-at-point ()
  (string-to-number (format-mode-line "%l"))
  )

(defun 7zr-line-number-at-pos ( pos )
  (save-excursion
    (goto-char pos)
    (7zr-line-number-at-point)
    )
  )
...
(setq 7zr-match-timestamp "^\\([0-9]\\{4\\}-[0-9]\\{2\\}-[0-9]\\{2\\} [0-9]\\{2\\}:[0-9]\\{2\\}:[0-9]\\{2\\}\\)") 
...
(setq 7zr-match-timestamp3 "\\(20[0-9]\\{2\\}\\)-\\([0-9]\\{2\\}\\)-\\([0-9]\\{2\\}\\) \\([0-9]\\{2\\}\\):\\([0-9]\\{2\\}\\):\\([0-9]\\{2\\}\\)")
;;  "regex pattern of timestamp, getting year month day hour minute second individually")

(setq 7zr-match-timestamp4 "\\(20[0-9]\\{2\\}\\)-\\([0-9]\\{2\\}\\)-\\([0-9]\\{2\\}\\) \\([0-9]\\{2\\}\\):\\([0-9]\\{2\\}\\)")
...
(setq 7zr-match-timestamp5 "\\(20[0-9]\\{2\\}\\)-\\([0-9]\\{2\\}\\)-\\([0-9]\\{2\\}\\)")
...
(defun 7zr-summary-goto-sha1 ()
  ...
  (if 7zr-disable-summary-goto-sha1
      (message "Missing grep.exe, which can be downloaded from http://gnuwin32.sourceforge.net/packages/grep.htm")
					; else
    (if (bound-and-true-p 7zr-awk-not-installed-for-windows)
	(message "awk, or rather gawk, must be installed for windows before you can run this command.")
					; else
...
; not be deleted
  ...
  (let
	...
	(beginning-of-line)
	(setq 7zr-consolidate-line_from (7zr-line-number-at-point))
	...
	(previous-line)
	(setq 7zr-consolidate-line_to (7zr-line-number-at-point))
	(setq point-to-minus-last-line (line-end-position))
	
	(goto-char (point-max))
	(beginning-of-line)
	(previous-line)
	(setq max-line (7zr-line-number-at-point))
	(when (< max-line 3)
...
(defun 7zr-summary-all-diffs-of-region-to-1-buffer ( from to )
  ...
  (let
	...
	(goto-char from)  ; from will actually be 1 up from selected from
	(beginning-of-line)
	(setq line_from (7zr-line-number-at-point))
...
;;	(forward-line 1)
	...
	(unless (looking-at "\\([0-9]+\\.[0-9]+\\)")
	  (if (or (= line_from (7zr-line-number-at-point))
		  (= 1         (7zr-line-number-at-point)))
	      (setq abort_function t)  ;; from is at last line
	    (previous-line))    ;; else assume that just end is at last line
	  )
	(looking-at "\\([0-9]+\\.[0-9]+\\)")		
	(setq to-patch (match-string-no-properties 1))
;;	(previous-line)
...
what format will be outputted."
  (let (format)
    (cond ((string-match-p "u" 7zr-diff-command)
	   (setq format "unified"))
	  ((string-match-p "c" 7zr-diff-command)
	   (setq format "context"))
	  ((string-match-p "y" 7zr-diff-command)
	   (setq format "side-by-side"))
	  ((string-match-p "e" yzr-diff-command)
...
(defun 7zr-diff-format-defaultp ()
  ...
  (if (and (not (string-match-p "u" 7zr-diff-command))
	   (not (string-match-p "c" 7zr-diff-command))
	   (not (string-match-p "y" 7zr-diff-command))
	   (not (string-match-p "e" 7zr-diff-command))
	   )
      t
    nil
    )
...
;  (setq 7zr-view-last-column 7zr-active-document_view-last-column )
  ...
  ;(setq 7zr-pointer-lastviewed_nearest_progeny 7zr-active-document_7zr-pointer-lastviewed_nearest_progeny)
...
;  (set (make-local-variable '7zr-active-document_7zr-summary-rev-at-point) 7zr-summary-rev-at-point)
  (setq 7zr-summary-rev-at-point 7zr-active-document_7zr-summary-rev-at-point)
  (setq 7zr-summary-linenum-at-point 7zr-active-document_7zr-summary-linenum-at-point)
  (when (bound-and-true-p 7zr-active-document_7zr-summary-lastlinenum-at-point)
    (setq 7zr-summary-lastlinenum-at-point 7zr-active-document_7zr-summary-lastlinenum-at-point)
    )
  (setq 7zr-revisions_original_buffer 7zr-active-document_7zr-revisions_original_buffer)
  (setq 7zr-revisions_original_buffer_name 7zr-active-document_7zr-revisions_original_buffer_name )
)
...
(defun 7zr-summary-view-revision ()
  (interactive)
  "Reconstruct and then view the revision pointed to by (point) in the 7zr-revisions_tmpbuffer summary buffer and call 7zr-reconstruct-rev-from-patches, unless we are looking at top most or bottom most line, or one up from bottom most line, in which case we'll open the files from this function instead of calling (7zr-reconstruct-rev-from-patches).  This function is called by the interactive command functions that select the next and previous tuples "
 ; (if (not (bound-and-true-p 7zr-active-document))
  ;    (message "This function should only be called from a 7z-revisions summary buffer")
    (setq 7zr-summary-view-revision_last_rev 7zr-pointer-lastviewed)
    (setq 7zr-summary-linenum-at-point (7zr-line-number-at-point))
...
;;;;;; viewing original version     
     ((looking-at 7zr-original-version)          ;; viewing original version
      (save-window-excursion
	(setq 7zr-summary-last-line (7zr-line-number-at-point))
	(shell-command (concat "7z e -aoa -o" (7zr-shell-quote-argument 7zr-temp-directory) " " (7zr-shell-quote-argument (concat 7zr-archive-directory 7zr-archive-name)) " " (7zr-shell-quote-argument 7zr-original-version)))  
	(7zr-rename-file-from-temp-if-exists (concat 7zr-original-version) (concat 7zr-temp-directory "rev" "0.0" "_of_" 7zr-original-version))
	;;	     (setq 7zr-revisions_lastbuffer (current-buffer))
	)
      (if (and (boundp '7zr-summary-last-line-last) (= 7zr-summary-last-line-last 3)) ;; meaning summary view pointer was traversing upward
	  (progn
	    (setq 7zr-reconstruct-dtag " -R -t ")   ; 
	    (setq 7zr-summary-view-revision_last_rev 7zr-pointer-lastviewed)
	    (setq 7zr-summary-view-revision_calc_linenum t)
	    (save-window-excursion
	      (shell-command (concat "7z e -aoa -o" (7zr-shell-quote-argument 7zr-temp-directory) " " (7zr-shell-quote-argument (concat 7zr-archive-directory 7zr-archive-name)) " " (7zr-shell-quote-argument 7zr-summary-view-revision_last_rev)))  
	      )
	    )
	   				; else
	(setq 7zr-summary-view-revision_calc_linenum nil)
	)
      (7zr-reconstruct_refresh-some-pointers "0.0" (7zr-line-number-at-point))
      (setq 7zr-pointer-lastviewed "0.0")
...
;;	   (find-file-read-only (concat 7zr-temp-directory  7zr-original-version))
      (find-file-read-only (concat 7zr-temp-directory  "rev0.0_of_" 7zr-original-version))
      (7z-revisions-mode 0)
      
      (set (make-local-variable '7zr-original-page) t)
	...
	(7zr-view-highlight-changes nil t)
      ...
      (when (bound-and-true-p 7zr-summary-view-revision_calc_linenum) 
	
	(setq 7zr-view-last-line (car (7zr-new-linenum-after-diff 7zr-view-last-line (concat 7zr-temp-directory 7zr-summary-view-revision_last_rev ) 7zr-reconstruct-dtag)))   ; 7zr-summary-view-revision_last_rev 
	...
	(when (bound-and-true-p 7zr-active-document_last-buffer)
      ...
      (message (concat "Archived on " (7zr-get-creation-date-from-original-version 7zr-original-version) " " (gethash "0.0" (buffer-local-value '7zr-notestab 7zr-active-document_last-buffer))))
      (7zr-view-mode))

	  
;;;;;;;;;;;; viewing latest revision
     ((looking-at 7zr-prepend-to-latest-revision)  ;; viewing latest revision
      (previous-line)
      (save-window-excursion
	(setq 7zr-summary-last-line (7zr-line-number-at-point))
	(7zr-reconstruct_refresh-some-pointers (number-to-string 7zr-patch-number) (7zr-line-number-at-point))
	(setq 7zr-summary-linenum-at-point (7zr-line-number-at-point))
...
;;	   (setq 7zr-revisions_lastbuffer (current-buffer))
      ...
      (setq 7zr-pointer-lastviewed_nearest_progeny (number-to-string 7zr-patch-number))
...
;	   (7zr-view-local-set-keys)
	...
	(7zr-goto-last-line-column)
	   ...
	   ;; (7zr-set-some-buffer-local-vars "7zr-summary-view-revision" 7zr-last-buffer)
      (set (make-local-variable '7zr-active-document_archive-directory-full) 7zr-archive-directory)
      (set (make-local-variable '7zr-latest-page) t)
      )


;;;;;;;;; viewing last revision
     ((looking-at (number-to-string 7zr-patch-number))   ;; viewing last revision
      (save-window-excursion
	nil
	(setq 7zr-summary-last-line (7zr-line-number-at-point))
	(shell-command (concat "7z e -aoa -o" (7zr-shell-quote-argument 7zr-temp-directory) " " (7zr-shell-quote-argument (concat 7zr-archive-directory 7zr-archive-name)) " " (7zr-shell-quote-argument (concat 7zr-prepend-to-latest-revision  7zr-original-version))))
...
;;	    (setq 7zr-revisions_lastbuffer (current-buffer))
      ...
      (setq 7zr-summary-last-line (7zr-line-number-at-point))
      (7zr-reconstruct_refresh-some-pointers (number-to-string 7zr-patch-number) (7zr-line-number-at-point))
      (if (and (boundp '7zr-summary-last-line-last) (= 7zr-summary-last-line-last (- (7zr-line-number-at-point) 2))) ;; meaning summary view pointer was traversing downward
	  (progn
	    (setq 7zr-reconstruct-dtag " -t ")   ; 
	    (setq 7zr-summary-view-revision_last_rev 7zr-pointer-lastviewed)
	    (setq 7zr-summary-view-revision_calc_linenum t)
	    (save-window-excursion
	      (shell-command (concat "7z e -aoa -o" (7zr-shell-quote-argument 7zr-temp-directory) " " (7zr-shell-quote-argument (concat 7zr-archive-directory 7zr-archive-name)) " " (7zr-shell-quote-argument (number-to-string 7zr-patch-number))))  
	      )
	    )
	   				; else
	(setq 7zr-summary-view-revision_calc_linenum nil)
      ...
      (7zr-reconstruct_refresh-some-pointers 7zr-pointer-lastviewed (7zr-line-number-at-point))
      (find-file-read-only (concat 7zr-temp-directory  "rev" (number-to-string 7zr-patch-number) "_of_" 7zr-original-version))
      (7z-revisions-mode 0)
      (when (bound-and-true-p undo-tree-mode)
	(undo-tree-mode 1))
...
;	   (when (bound-and-true-p 7zr-last-column)  ; this temporarily fixes the error that always arises when trying to view revision from the very last line in the summary view.
      ...
      (when (not (bound-and-true-p 7zr-summary-view-revision_calc_linenum))
	(7zr-goto-last-line-column)
	)
;	     )  
      (7zr-set-some-buffer-local-vars "7zr-summary-view-revision" 7zr-last-buffer)
...
;	   (7zr-parse-standard-diff-type t)
	...
	(setq 7zr-view-last-line (car (7zr-new-linenum-after-diff 7zr-view-last-line (concat 7zr-temp-directory (number-to-string 7zr-patch-number) ) " ")))   
	    ; (setq 7zr-view-last-line (buffer-local-value '7zr-last-line 7zr-active-document_last-buffer))
	(when (bound-and-true-p 7zr-active-document_last-buffer)
	  (with-current-buffer 7zr-active-document_last-buffer
	    (set (make-local-variable '7zr-last-line) 7zr-view-last-line)
...
(defun 7zr-summary-quit ()
  ...
  (when (and (bound-and-true-p 7zr-active-document_buffer)
    ...
    (setq 7zr-goto-this-line-number (7zr-line-number-at-point))
...
; (7zr-delete-file-if-exists (concat 7zr-temp-directory 7zr-pointer-lastviewed))
    ...
    (kill-buffer)
    (when (buffer-live-p 7zr-revisions_tmpbuffer)
...
(defun 7zr-view-quit_view_diff ()
  ...
  (let (current_line_num (not_found t) diff_line_num (diff_line_num_highest 0) (diff_line_num_highest_pos 0))
    (setq current_line_num (7zr-line-number-at-point))
    (7zr-save-last-line-column)
    (kill-buffer)
...
(defun 7zr-save-last-line-column ()
  (setq 7zr-view-last-line (7zr-line-number-at-point))
  (setq 7zr-view-last-column (current-column))
  (when (and (bound-and-true-p 7zr-active-document_last-buffer) (buffer-live-p 7zr-active-document_last-buffer))
    (with-current-buffer 7zr-active-document_last-buffer
      (set (make-local-variable '7zr-last-line) 7zr-view-last-line)
      (set (make-local-variable '7zr-last-column) 7zr-view-last-column)
      )
...
(defun 7zr-goto-last-line-column ()
  (when (or
	 (bound-and-true-p 7zr-active-document_last-buffer)
	 (bound-and-true-p 7zr-original-page))
...
(defun 7zr-view_next_page ()
  ...
  (interactive)
  (if (7zr-not-viewing-p "7zr-summary-view-revision")       
      (message "Must be viewing a 7z-revisions revision to use this function.")

    (when (bound-and-true-p evil-mode)
      (setq 7zr-preserved-evil-emacs-state-p (evil-emacs-state-p)))
    (7zr-save-last-line-column)
...
;    (7zr-goto-last-line-column)
    (when (and
	   ...
	   7zr-preserved-evil-emacs-state-p)
      (evil-emacs-state))
...
(defun 7zr-view_previous_page ()
  ...
  (if (7zr-not-viewing-p "7zr-summary-view-revision")      
      (message "Must be viewing a 7z-revisions revision to use this function.")
    (when (not (bound-and-true-p 7zr-original-page))  ;; stop at rev 0.0
      (when (bound-and-true-p evil-mode)
	(setq 7zr-preserved-evil-emacs-state-p (evil-emacs-state-p)))
...
(setq 7zr-view-mode-map 
  (let ((map (make-sparse-keymap)))
   ...
   (define-key map [menu-bar 7zr-view quit_view_diff]
    ...
    (define-key map [menu-bar 7zr-view rev_last_changed]
      '(menu-item "Goto rev that changed region" 7zr-goto-rev-region-last-changed
                  :help "Goto the past revision that changed selected region."))
...
(defun 7zr-view-raw-diff_next_page ()
  ...
  (if (7zr-not-viewing-p "7zr-summary-view-raw-diff-file")       
      (message "Must be viewing a 7z-revisions diff file in order to use this function.")
  ...
  (save-excursion
	...
	(let* ((a-begin (string-to-number (buffer-substring (match-beginning 1)
	     ...
	     ; (hunk-b (replace-regexp-in-string "^[<|>] " "" hunk-b)
	       ...
	       c-begin c-end c-begin-pt c-end-pt column_current column_last difference-ranges highlight_diff_list)
 ...
 ;      (when (and (not parse-ancestorp) (string= diff-type "d"))
   ...
   ;; try this and see what happens
	  ...
	  (dotimes (line a-num_lines line)
	    ...
	    (setq a-begin (7zr-line-number-at-point))
	    (beginning-of-line)
	    (forward-char 2)
	    ...
	    ;; (setq column_current (- (current-column) 2))
	      (setq column_current (current-column))
	      ...
	      (setq a-line_pos (vconcat a-line_pos (make-vector 1 (7zr-line-number-at-point))))
	      (setq a-offset_point_last (point))
	  ...
	  (dotimes (line b-num_lines line)
	    ...
	    ;(setq column_current (- (current-column) 2))
	      ...
	      (setq b-line_pos (vconcat b-line_pos (make-vector 1 (7zr-line-number-at-point))))
	      (setq b-offset_point_last (point))
	      (setq column_last column_current)
	      ) ; while
	    ) ; dotimes
...
;	(set-buffer 7zr-parse-standard-diff-type_lastbuffer)
	  ...
	  (when (not (string= diff-type "d"))
	    ...
	    ;(pp a-words)
	    ;(pp b-words)
	    ;(pp "difference-range1\n")
	    ;(pp difference-ranges)
	    (7zr-map-difference-ranges-to-point-ranges difference-ranges a-line_pos a-column_pos t)
	  ;(setq highlight_diff_list (append highlight_diff_list (7zr-map-difference-ranges-to-point-ranges difference-ranges a-line_pos a-column_pos t)))
	    
	    (setq difference-ranges (7zr-longest-common-word-subsequence-difference-ranges a-words b-words t))
	  
	    ;(pp "difference-range2\n")
	    ;(pp difference-ranges)
	    (7zr-map-difference-ranges-to-point-ranges difference-ranges b-line_pos b-column_pos t)   ;; ok why is this changing the 1st line when set to t?
	  
;	  (setq highlight_diff_list (append highlight_diff_list (7zr-map-difference-ranges-to-point-ranges difference-ranges b-line_pos b-column_pos nil)))
	    ) ; when not deleting lines
...
(setq 7zr-view-raw-diff-file-mode-map 
  (let ((map (make-sparse-keymap)))
...
(setq 7zr-date-time-format "%Y-%m-%d_%H:%M:%S")
...
(defun 7zr-string-ends-with-p (string suffix)
  ...
  (and (string-match-p (rx-to-string `(: ,suffix eos) t)
                     string)
       t))
 ...
 (defun 7zr-toggle-highlight ()
   ...
   (if (and
	...
	(7zr-string-starts-with-p (buffer-name (current-buffer)) "7z-revisions_of_"))
       (hl-line-mode 1)
     (hl-line-mode 0)
     )
   )
...
(defun 7zr-toggle-auto-tags ()
  "In 7z-revisions-mode special tags can be presented in the first 7777 characters of the buffer which can specify things like the location of the 7zr archive, or the archive extension, and can also be used as an achor point for information to be inserted, such as the current revision number, or the sha1 hash value of the last revision."
   (interactive)
...
(setq 7zr-color-revs t)
(setq 7zr-bkgrndclr->frgrndclr
      '(("red" . "white")
	("orange" . "black")
	("yellow" . "black")
	("green" . "black")
...
(defun 7zr-input-note ( color )
  ...
  (interactive "P")
  (setq 7zr-note (read-string (concat "Enter " (car (rassoc color 7zr-color->number )) " note:" )))
...
(defun 7zr-edit-note ( rev_string default_note )
    (7zr-escape-quotes (read-from-minibuffer (concat "Edit note rev " rev_string ", C-c C-c when finished, C-] to cancel:\n") default_note 7zr-minibuffer-input-mode-map))
    )
...
(defun 7zr-sans-archive-extension (filenamestring)
  ...
  (let ((len (length 7zr-archive-extension)))
    (if (string= (substring filenamestring (- len) nil) 7zr-archive-extension)
...
(defun 7zr-what-is-last-revision ()
  ...
  (let (7zr-buffer 7zr-buffer-filename 7zr-patch-number 7zr-revisions_lastbuffer 7zr-revisions_tmpbuffer2 7zname patch last-revision)
	...
	(message (concat "Archive " 7zr-archive-name " has not yet been created yet, neither via 7z-revisions-mode nor by typing M-x 7zr-commit."))
      ...
      (if (eql (string-match-p "No files to process" (shell-command-to-string (concat "7z e -aoa -o" (7zr-shell-quote-argument 7zr-temp-directory) " "  (7zr-shell-quote-argument (concat 7zr-archive-directory 7zr-archive-name))  " " (7zr-shell-quote-argument 7zr-archive-created-by-message)))) nil)
	  (progn
	    (7zr-delete-file-if-exists (concat 7zr-temp-directory 7zr-archive-created-by-message))
...
;	  (switch-to-buffer 7zr-revisions_tmpbuffer2)
	  ...
	  (save-window-excursion
	    ...
	    (while (re-search-forward "^\\([0-9]\\{4\\}-[0-9]\\{2\\}-[0-9]\\{2\\} [0-9]\\{2\\}:[0-9]\\{2\\}:[0-9]\\{2\\}\\) ..... [ 0-9]+ [ 0-9]+  \\(.*\\)" nil t)
	      ...
	      (beginning-of-line)
	      (when 
		  (not (string-match-p "[a-zA-Z]+" 7zname))   ; else it must be a number to be a patch, and so discard the rest
...
(defun 7zr-what-column-at-point ()
;;  (string-to-number (format-mode-line "%c"))
  (current-column)
  )

(defun 7zr-what-column-at-pointDEFUNCT (&optional detail)
  "This is a modified what-cursor-position, and returns the column number of the character at point."
  ;(interactive "P")
...
(defun 7z-revisions ()
 ...
 ; (setq cntr 0) ; debug
  ...
  (unless (file-directory-p 7zr-temp-directory)
    (make-directory 7zr-temp-directory t))
;;  (shell-command (concat "touch " 7zr-temp-directory "0.0"))
  (7zr-populate-initial-global-vars) 
  (7zr-set-archive-directory-full 7zr-archive-directory)
  (setq 7zr-revisions_original_buffer (current-buffer))
  (setq 7zr-revisions_original_buffer_name (buffer-name (current-buffer))) 
  ...
  (setq 7zr-buffer-misc (concat 7zr-revisions_tmpbuffer-default_prefix_name  (file-name-nondirectory (buffer-file-name (current-buffer)))))
  (if (buffer-live-p (get-buffer 7zr-buffer-misc))
      (switch-to-buffer 7zr-buffer-misc)
					; else
    (if (not (file-exists-p (concat 7zr-archive-directory 7zr-archive-name)))
	(message (concat "Archive " 7zr-archive-name " has not yet been created yet, neither via 7z-revisions-mode nor by typing M-x 7zr-commit."))
      (if (7zr-is-a-valid-archive-p)
	  (progn
	    ...
	    (setq 7zr-revisions_tmpbuffer2 (generate-new-buffer-name (concat 7zr-revisions_tmpbuffer-default_prefix_name 7zr-buffer)))
...
;	      (next-line)
 ...
 ;    (error "Not a valid 7z-revisions.el archive file")
	    ...
	    ;; loading hash table from here doesnt seem to work
...
;		 (not (string= (format-mode-line "%l") "1"))
		   (not (equal (7zr-line-number-at-point) 1))
		   )
	      (beginning-of-line)
	    ...
	    (goto-char (point-max))
	    (setq 7zr-revisions_tmpbuffer_lines (7zr-line-number-at-point))
	    ...
	    (end-of-buffer)
	    (previous-line)
	    ;; local variables set after this point seem to work
	    (set (make-local-variable '7zr-summary-page) t)
...
(defun 7zr-dired-7z-revisions ()
  "This function can be called from dired using the C-c z key combination.  This function finds the 7zr-original-version variable and displays all the revisions in an interactive buffer, which is stored in the variable 7zr-revisions_tmpbuffer, with names of patches, discarding any filenames in the archive that arent numbers per number-or-marker-p, and also gets the 7zr-patch-number.  When a revision is selected and the enter key is pressed, or the j key in the case of displaying the respective diff files, then the (7zr-summary-view-revision) function takes over."
  (interactive)
					; (setq cntr 0) ; debug
...
;    (setq 7zr-active-document_buffer (current-buffer))
 ;   (setq 7zr-active-document 7zr-active-document_buffer)
    (setq 7zr-buffer (file-name-nondirectory (7zr-sans-archive-extension (dired-get-file-for-visit))))
    ...
    (setq 7zr-buffer-misc (concat 7zr-revisions_tmpbuffer-default_prefix_name  7zr-buffer))
    
    (let ((tmpdir (concat 7zr-temp-directory (7zr-random-str 30))) tmpfiles tmpfiles2 newfile_name newfile)
      (save-window-excursion  
	...
	(if (not newfile_name)
	  ...
	  (kill-buffer newfile)
	  (7zr-delete-file-if-exists  newfile_name)
    ...
    ; (if (not (string= 7zr-buffer-filename-extension 7zr-archive-extension-suffix))
      ...
      (if (7zr-is-a-valid-archive-p) 
	  (progn
	 ; (7zr-extract-a-file-to-temp-directory 7zr-prepend-to-latest-revision 7zr-original-version)
	    (setq 7zr-summary-last-line-last 0)
	    (set (make-local-variable '7zr-window)   "dired-buffer")
	    (setq 7zr-revisions_lastbuffer (current-buffer))
	    (setq 7zr-revisions_tmpbuffer (generate-new-buffer-name (concat "7zr-revisions_of_" 7zr-buffer)))
 ...
 ;	  (setf (buffer-local-value '7zr-active-document 7zr-revisions_tmpbuffer) '7zr-buffer)
	    (set (make-local-variable '7zr-active-document_buffer) (current-buffer))
	    (with-current-buffer 7zr-revisions_tmpbuffer
	      (set (make-local-variable '7zr-active-document) 7zr-revisions_original_buffer ))
...
;      (setq timetuple (apply #'encode-time (parse-time-string 7zdatetime)))
		  ...
		  (if (and (7zr-string-starts-with-p 7zname 7zr-original-version)  ; find original version
			   (not (string= 7zname 7zr-archive-created-by-message))  ; not the archive identifier
			   )
		      (progn
...
;	      (next-line)
 ...
 ;    (error "Not a valid 7z archive file")
     ...
     ;	  (delete-backward-char 1)
	    ...
	    (end-of-buffer)
	    (previous-line)
	    (7zr-set-some-buffer-local-vars "7zr-dired-7z-revisions"  7zr-revisions_original_buffer)
	    (set (make-local-variable '7zr-summary-page) t)
	    (set (make-local-variable '7zr-active-document_buffer) 7zr-revisions_original_buffer) 
	      
	    (switch-to-buffer 7zr-revisions_tmpbuffer)
	    (set (make-local-variable '7zr-active-document_7zr-revisions_original_buffer) 7zr-revisions_original_buffer)
	    (set (make-local-variable '7zr-active-document_7zr-revisions_original_buffer_name) "hithere")
	    (set (make-local-variable '7zr-active-document_7zr-patch-number) 7zr-patch-number)
	    (set (make-local-variable '7zr-window) "7zr-dired-7z-revisions")
	    (7zr-revisions-load-notestab-unsecure)
	   ; (when 7zr-track-md5sum-hashes-p
	    (7zr-revisions-load-hashtable-unsecure)
					;  )
...
(defun 7zr-random-str ( len )
  "Returns a string consisting of lower and upper case english alphabetical characters of length LEN"
  (let* (result (alphab [ 65 66 67 68 69 70 71 72 73 74 75 76 77 78 79 80 81 82 83 84 85 86 87 88 89 90 97 98 99 100 101 102 103 104 105 106 107 108 109 110 111 112 113 114 115 116 117 118 119 120 121 122 ]) (alphab_len (length alphab)) (result_stack '()))
    (dotimes (x len (concat result_stack)) (push (aref alphab (% (abs (random)) alphab_len)) result_stack))
    )
  )
...
(defun 7zr-summary-highlight-all-revs-with-notes ()
  (interactive)
  (7zr-summary-view-revision_get-buffer-local-vars)
  (when (eql (string-match-p "No files to process" (shell-command-to-string (concat "7z e -aoa -o" (7zr-shell-quote-argument 7zr-temp-directory) " "  (7zr-shell-quote-argument (concat 7zr-archive-directory 7zr-archive-name))  " " (7zr-shell-quote-argument 7zr-archive-created-by-message)))) nil)

    (let (saved-point newbuffer newbuffer-name lastbuffer pos_list start_pos end_pos tmp rev rev_list)
      (setq lastbuffer (current-buffer))
      (7zr-extract-a-file-to-temp-directory 7zr-prepend-to-notes-file 7zr-original-version 7zr-notes-file-extension)
      (setq newbuffer-name (generate-new-buffer-name (concat "7zr-newbuffer_" "somev")))
      (get-buffer-create newbuffer-name)
      (set-buffer newbuffer-name)
      (setq newbuffer (current-buffer))
      (goto-char (point-min))
      (insert-file-contents (concat (7zr-shell-quote-argument 7zr-temp-directory) 7zr-prepend-to-notes-file 7zr-original-version 7zr-notes-file-extension))
      (goto-char (point-min))
      (while (re-search-forward "^(puthash \"\\([[:digit:]]+\\.[[:digit:]]+\\)\"" nil t)
	...
	(push rev pos_list)
      ...
      (save-excursion
	(read-only-mode 0)
	(while (setq rev (pop pos_list))
	  (goto-char (point-min))
	  (when (re-search-forward (concat "^" (7zr-escape-dot rev)) nil t)
	    (setq start-pos (match-beginning 0))
	    (setq end-pos (match-end 0))
	    (put-text-property start-pos end-pos 'font-lock-face '(:foreground "red" :background "yellow"))
	    )
	  ...
	  (mapc
	   (lambda (x)
	     ...
	     (let ((case-fold-search nil))
	       (while (re-search-forward (upcase (car x)) nil t)
		 (save-excursion
		   (save-match-data
		     (when (re-search-backward "^(puthash \"\\([[:digit:]]+\\.[[:digit:]]+\\)\"" nil t)
		       (push (match-string-no-properties 1) rev_list)
		       )
		     )
		   )
		 )
	     ...
	     (while (setq rev (pop rev_list))
	       (goto-char (point-min))
	       (when (re-search-forward (concat "^" (7zr-escape-dot rev)) nil t)
		 (setq start-pos (match-beginning 0))
		 (setq end-pos (match-end 0))
		 (put-text-property start-pos end-pos 'font-lock-face `(:foreground ,(cdr x) :background ,(car x)))
		 )
	       
	       )
	     )
	   7zr-bkgrndclr->frgrndclr
	   )
	  )
	)
      (set-buffer 7zr-revisions_tmpbuffer)
...
(defun 7zr-summary-previous-line ()
  ...
  (if (looking-at "\\([0-9]+\\.[0-9]*\\)")
    (message (gethash (match-string-no-properties 1) 7zr-notestab))
					; else
    (if (= (7zr-line-number-at-point) 1)
	(message (gethash "0.0" 7zr-notestab))
      )
    )
...
(defun 7zr-is-a-valid-archive-p ()
  (let (validp)
    (save-window-excursion
      (when (file-exists-p (concat 7zr-archive-directory 7zr-archive-name))
	(setq validp (eql (string-match-p "No files to process" (shell-command-to-string (concat "7z e -aoa -o" (7zr-shell-quote-argument 7zr-temp-directory) " "  (7zr-shell-quote-argument (concat 7zr-archive-directory 7zr-archive-name))  " " (7zr-shell-quote-argument 7zr-archive-created-by-message)))) nil))
	(7zr-delete-file-from-temp-if-exists 7zr-archive-created-by-message)
	)
      )
    validp
    )
...
(defun 7zr-regenerate-all-hashes-from-diffs ()
  "In case the hash file somehow gets hozed, you can always regenerate it.  Althouth, the old hashes are not deleted from the hash file, but rather appended to the end of the file, overwriting their prior values when the hash file is loaded."
  (interactive)
  (unless (file-directory-p 7zr-temp-directory)
    (make-directory 7zr-temp-directory t))
  (if (and
       (bound-and-true-p 7zr-window)
       (string= 7zr-window "7zr-dired-7z-revisions")
       )
      (progn
	(7zr-summary-view-revision_get-buffer-local-vars)
	(7zr-set-archive-directory-full 7zr-archive-directory)
					;(7zr-populate-initial-global-vars_dired)
	(setq 7zr-buffer-misc 7zr-active-document_7zr-revisions_tmpbuffer)

	(setq 7zr-revisions_original_buffer_name 7zr-buffer) 
	)
					; else
    (7zr-populate-initial-global-vars)
  ...
  (when (buffer-live-p (get-buffer 7zr-buffer-misc))
    (switch-to-buffer 7zr-buffer-misc)
    (7zr-summary-quit))		
  (if (not (7zr-is-a-valid-archive-p))
      (message "This function can only be called from a document associated with a 7z-revisions archive")
					; else
    
    (when (yes-or-no-p (concat "Regenerate all hashes for " 7zr-archive-directory 7zr-archive-name ", really?"))
      (let (archive_info revs_tmpbuffer_name  revs_tmpbuffer (orig_buffer 7zr-revisions_original_buffer) patch-number_str lines current-patch (save-dir default-directory) hash hashes_str hash patched_rev)
	(setq archive_info (7zr-list-revs 7zr-revisions_original_buffer_name))
	
	(setq 7zr-original-version (car archive_info))
	(setq revs_tmpbuffer_name (car (cdr archive_info)))
	(setq 7zr-patch-number (car (cdr (cdr archive_info))))
	(setq patch-number_str (number-to-string  7zr-patch-number))
	(setq lines (nth 3 archive_info))
	(setq hashes_str (concat "\n;; The following " (number-to-string lines) " lines ending in rev " patch-number_str " were added on " (7zr-current-date-time) "\n"))
	(setq 7zr-misc-value lines)
	(set-buffer revs_tmpbuffer_name)
        (setq revs_tmpbuffer (current-buffer))
	(goto-char (point-min))
	(forward-line)
;(setq saved-dir default-directory)
	(7zr-extract-a-file-to-temp-directory nil 7zr-original-version)
	(7zr-rename-file-from-temp-if-exists  7zr-original-version "wip")
	(setq hashes_str (concat hashes_str "(puthash \"0.0\" \"" (7zr-sha1-of-file-in-temp-directory "wip") "\" 7zr-hasht)\n"))
	(cd 7zr-temp-directory)
	(switch-to-buffer revs_tmpbuffer)
	(while (looking-at "\\([0-9]+\\.[0-9]*\\)[[:blank:]]")
	  (setq current-patch (match-string-no-properties 1))
	  (7zr-extract-a-file-to-temp-directory nil current-patch) 
	  (setq hash (7zr-forward-patch-in-temp-directory "wip" current-patch))
	  (copy-file "wip" (concat "rev" current-patch "wip") 'ok-if-already-exists )
	  (setq hashes_str (concat  hashes_str "(puthash \"" current-patch   "\" \"" hash "\" 7zr-hasht)\n"))
	  (7zr-delete-file-from-temp-if-exists current-patch)
	  (message (concat current-patch " of " patch-number_str))
	  (forward-line)
	  )  ; while
	(set-buffer orig_buffer)
	(cd save-dir)
	(7zr-add-to-hashfile hashes_str)
	(7zr-extract-a-file-to-temp-directory 7zr-prepend-to-latest-revision 7zr-original-version )
	(if (string= hash (7zr-sha1-of-file-in-temp-directory (concat 7zr-prepend-to-latest-revision 7zr-original-version)))
	    (message "regenerated hashvalues are all valid.")
	  (message "invalid hashes!")
	  )
	(7zr-delete-file-from-temp-if-exists (concat 7zr-prepend-to-latest-revision 7zr-original-version))
	(kill-buffer revs_tmpbuffer_name) 
	(set-buffer orig_buffer)
	)
      )
    )    
...
(defun 7zr-sha1-of-file-in-temp-directory ( file_in_temp_directory )
  "Returns the sha1 of a file that exists in the directory contained in the variable 7zr-temp-directory, which is ussually just temporary-file-directory which is generally /tmp/"
  (let (sha1 (saved-buffer (current-buffer))(saved-dir default-directory) (tmpbuffer_name (generate-new-buffer-name "sha1-generate")) tmpbuffer)
    (save-window-excursion

      (cd 7zr-temp-directory)
      (when (file-exists-p file_in_temp_directory)
	(setq tmpbuffer (get-buffer-create tmpbuffer_name))
	(set-buffer tmpbuffer)
	(goto-char (point-min))
	(insert-file-contents file_in_temp_directory)
	(setq sha1 (sha1 tmpbuffer))
...
(defun 7zr-revisions-load-hashtable-unsecure ()
  ...
  (save-window-excursion
    ...
    (if (not (bound-and-true-p 7zr-summary-page))
	(error "this function can only be used from the 7z-revisions summary page")
...
;; (defun 7zr-time-view_previous_page ()
;;   (interactive)
;;   (pp (measure-time (7zr-view_previous_page)))
;;   )

(defun 7zr-revisions-validate-rev-p ( rev-pointer )
"Check revision against hash"
  (let (validp (rev-hash "") (hash-from-table "") (hash-rev-to-check-against ""))
    ...
    ;;   (setq hash-rev-to-check-against rev-pointer)
	  ...
	  (if (not 7zr-track-md5sum-hashes-p)
	    ...
	    (setq rev-hash (7zr-string-trim-final-newline (7zr-string-trim-final-newline (shell-command-to-string (concat 7zr-sha1sum-command " " (7zr-shell-quote-argument (concat 7zr-temp-directory "rev" rev-pointer "_of_"  7zr-original-version)) 7zr-sha1sum-post-command (7zr-awk-cmd-string 1)))))) ;; comment out
	  ;;  (setq rev-hash (7zr-sha1-of-file-in-temp-directory (concat 7zr-temp-directory "rev" rev-pointer "_of_"  7zr-original-version)))  ;; comment out
	    (setq hash-from-table (gethash hash-rev-to-check-against 7zr-hasht))
	    (setq validp (string= rev-hash hash-from-table))
	    (when (not validp)
	      (message (concat "7zr invalid pointer " rev-pointer ))
	      )
	    validp

	    )
	(message (concat "7zr invalid pointer " rev-pointer " as file not found."))
	nil
	) ; if file exists
      ) ; let
  )
...
(defun 7zr-apply-patch-reverse ( wip_buffer linenum_param )
  ...
  (let ( (first-hunk-encountered-backward-that-changes-linenum-p t) (diff-buffer (current-buffer)) diff-string (linenum_offset 0) (hunknum 0) (hunknum-that-changed-linenum 0) last_match_pos current_match_pos)
   ...
   ;; try this and see what happens
	...
	(when first-hunk-encountered-backward-that-changes-linenum-p
	  (if (and
...
;;	  (setq a-end-saved a-end b-end-saved b-end) ;debug
	    (when (> linenum_param b-end )
	      (setq first-hunk-encountered-backward-that-changes-linenum-p nil)
...
;    (kill-buffer diff-buffer)
    ...
    (if (and 7zr-misc_No-newline-pos1 (not 7zr-misc_No-newline-pos2))
					;(backward-delete-char-untabify 1)  ;; debug
...
(defun 7zr-apply-patch-forward ( wip_buffer linenum_param )
  "This is an elisp implementation of the unix patch -p0 command, which only works on diffs of unified format, created by the unix command diff -Na.  Returns the new calculated line number.  When calling this function the current-buffer must be that of a patch/diff file."
  (let ((first-hunk-encountered-backward-that-changes-linenum-p t) (diff-buffer (current-buffer)) diff-string (linenum_offset 0) (hunknum 0) (hunknum-that-changed-linenum 0) last_match_pos current_match_pos)
    (set-buffer diff-buffer)
...
;	  (backward-char 1))
	...
	(setq hunknum (1+ hunknum))
					; track current line number changes
	(when (and
	       first-hunk-encountered-backward-that-changes-linenum-p
	      (> linenum_param a-end ))
	  (setq first-hunk-encountered-backward-that-changes-linenum-p nil)
...
(defun 7zr-reconstruct-get-max-patch-stringDEFUNCT ()
  " sets  7zr-reconstruct-max-patch-string and 7zr-revisions_tmpbuffer_lines which are max number of lines minus 1"
  (if (not (bound-and-true-p 7zr-summary-page))
      (error "this function can only be called from 7z-revisions summary view")
  (setq 7zr-reconstruct-save-point (point))
  (goto-char (point-max))
  (previous-line)
  (beginning-of-line)
  (if (looking-at "\\([0-9]+\\.[0-9]*\\)[[:blank:]]") 
      (progn
	(setq 7zr-reconstruct-max-patch-string (match-string-no-properties 1))
...
(defun 7zr-reconstruct-get-max-patch-string ()
  (setq 7zr-revisions_tmpbuffer_lines 7zr-active-document_7zr-revisions_tmpbuffer_lines)
  (setq 7zr-patch-number 7zr-active-document_7zr-patch-number)

  (setq 7zr-reconstruct-max-patch-string (number-to-string 7zr-patch-number))
  )
...
;	  (setq 7zr-patch-command-called-x-times (incf 7zr-patch-command-called-x-times))  ;debug
	  (when 7zr-track-md5sum-hashes-p 
	    ...
	    (setq hash-from-file (7zr-sha1-of-file-in-temp-directory 7zr-prepend-to-reconstruct_wip))
...
(defun 7zr-new-linenum-after-diff ( linenum_param diff-file direction_string )
  "Returns list, first element of which is the new line number calculated after a number of lines are added above the given linenum_param, or deleted, in the case of a negative number, as would result from patching from the diff-file. The 2nd list element returned is hunk number that resulted in a change to the linenum in question, 0 if unchanged.  The direction_string parameter is similar to the parameter given to the patch command, e.g. -R for reverse.  The next step after calling this function is typically to call  (7zr-goto-last-line-column) to actually go to that line."
  (interactive)
  (let (hunknum-that-changed-linenum hunknum n_diffbuffer n_lastbuffer n_diff_list)
    (setq n_lastbuffer (current-buffer))
    (save-window-excursion
      (if (not (file-exists-p diff-file))
	  (progn
	    (message (concat "file doesnt exist: " diff-file))
	    (list 1 0)  ; return same line as arg
	    )
					; else
	...
	(while (re-search-forward 7zr-match-diff-standard-line nil t)
	  ...
	  (let* ((a-begin (string-to-number (buffer-substring (match-beginning 1)
		 ...
		 (a-end  (let ((b (match-beginning 3))
			   ...
			   (if b
			       (string-to-number (buffer-substring b e))
			     a-begin)))
	     ;;  (diff-type (buffer-substring-no-properties (match-beginning 4) (match-end 4)))
		 (b-begin (string-to-number (buffer-substring (match-beginning 5)
							      (match-end 5))))
		 (b-end (let ((b (match-beginning 7))
			      (e (match-end 7)))
	 ...
	 ;; 	    b-end (1+ b-end))))
	  ...
	  ;;   (if (string-equal diff-type "d")
	  ;;  	(setq  b-end b-begin)))
	    (if (string-match-p "R" direction_string)
		(if (and
		     (>= linenum_param b-begin )
		     (<= linenum_param b-end ))
		    (setq hunknum-that-changed-linenum hunknum)
...
;	    (setq beginning_line (string-to-number (format-mode-line "%l")))  ;  older emacs versions dont have (current-line)
	    (setq beginning_line (7zr-line-number-at-point))
	    (goto-char (point-max))
	    (when (eobp)
	      (previous-line))
	    (previous-line 2)
	    (end-of-line)
	    (setq end_line (7zr-line-number-at-point))
...
;    (setq 7zr-buffer-without-extension (7zr-sans-extension filename))
	...
	(if (7zr-string-starts-with-p 7zname 7zr-prepend-to-latest-revision)  ; find latest revision
	  ...
	  (when 
	      (not (string-match-p "[a-za-z]+" 7zname))   ; else it must be a number to be a patch, and so discard it if it contains a letter
	    (setq patch (string-to-number 7zname))
...
(defun 7zr-list-revs ( filename )
  ...
  (let ( (tmp_patch_number 1.0) 7zr-buffer-without-extension 7zr-buffer-without-extension 7zr-list-revs_lastbuffer 7zr-list-revs_tmpbuffer 7zr-list-revs_tmpbuffer2 (ofile 7zr-archive-name)
			 files file sum col timetuple savepoint reached-minuses patch highest-patch 7zdatetime 7zsize 7zname (lines 0))
	...
	(if (and (7zr-string-starts-with-p 7zname 7zr-buffer-without-extension)  ; find original version
	  ...
	  (if (7zr-string-starts-with-p 7zname 7zr-prepend-to-latest-revision)  ; find latest revision
		...
		(not (string-match-p "[a-zA-Z]+" 7zname))   ; else it must be a number to be a patch, and so discard the rest
	      ...
	      (setq patch (string-to-number 7zname))
	      (when (> patch tmp_patch_number)            ; if patch is higher it becomes highest
		(setq tmp_patch_number patch)	  
		)
...
;	   (not (string= (format-mode-line "%l") "1"))
    ...
    (goto-char (point-max))
    (setq 7zr-list-revs_tmpbuffer_lines (7zr-line-number-at-point))
    (insert 7zr-latest-revision)
    (insert-tab)
    (insert 7zr-latest-revision_size)
    (insert-tab)
    ...
    (when (> 7zr-patch-number 1)
      (7zr-summary-sort-1))
    (goto-char (point-min))
    (setq 7zr-patch-number tmp_patch_number)
    ...
    (list 7zr-original-version  7zr-list-revs_tmpbuffer 7zr-patch-number (+ lines 2)) ; returns a string and a buffer name as string, and a number and a number
    ) ;let  
  )               ; 7zr-list-revs
...
(defun 7zr-line-last-changed-on ()
  ...
  (interactive)
  (let ( find-archive-name (linenum (7zr-line-number-at-point)) linenum_obj hunk_that_changed_line list-revs_obj revs-buffer rev rev-point pointer current-patch (saved-point-pos (point)) hash-of-file hash-from-table datetime_string 7zr-lastbuffer 7zr-buffer-file-name return_message)
	...
	(if (string-match-p "No files to process" (shell-command-to-string (concat "7z e -aoa -o" (7zr-shell-quote-argument 7zr-temp-directory) " "  (7zr-shell-quote-argument find-archive-name)  " " (7zr-shell-quote-argument 7zr-archive-created-by-message))))
	    (error "Not a valid 7z-revisions.el archive file")
	; else
	  ...
	  (if  (not (setq list-revs_obj (7zr-list-revs 7zr-buffer)))
	    ...
	    (shell-command (concat "7z e -aoa -o" (7zr-shell-quote-argument 7zr-temp-directory) " " (7zr-shell-quote-argument (concat 7zr-archive-directory 7zr-archive-name)) " " (7zr-shell-quote-argument (concat 7zr-prepend-to-latest-revision  7zr-original-version)))) 
	    (setq 7zr-line_not_changed t)
	    (setq hunk_that_changed_line 0)
	    (message "This might take a while...")
	    (while 7zr-line_not_changed
...
(defun 7zr-goto-rev-region-last-changed_get_initial_text ( linenum_begin lines )
  (let (char_begin char_end changed_txt)
    (save-excursion
      (goto-char (point-min))
      (forward-line (1- linenum_begin))
      (setq char_begin (point))
      (forward-line lines)
      (setq char_end (line-end-position))
      (goto-char char_begin)
  
      (setq changed_txt (buffer-substring-no-properties char_begin char_end))
      (message (concat "initial text: " changed_txt))
     changed_txt
      )
    )
  )

(defun 7zr-goto-rev-region-last-changed_get_cmp_text( lines )
  (let (line_at_point linenum_begin linenum_end cmp saved-pos)
    
 ;; (goto-char (point-min))
 ;; (forward-line (1- 7zr-goto-rev-region-last-changed_linenum_begin))
    ;(setq 7zr-goto-rev-region-last-changed_buffer (current-buffer))
    (setq line_at_point (7zr-line-number-at-point))
    (setq linenum_begin line_at_point)
    (setq saved-pos (point))
    (save-excursion

      (forward-line lines)
  ;;    (setq linenum_end (7zr-line-number-at-point))
      (setq cmp (buffer-substring-no-properties saved-pos (line-end-position)))
      (message (concat "cmp text: " cmp))
      cmp
      )
    )
  )

(defun 7zr-goto-rev-region-last-changed ()
  "Jumps to the revision and line of where the lines in a selected region have  last been modified (not the line numbers, per se, but the content at the given line numbers, which may have occupied a different line number in prior revisions because of lines deleted and/or removed above it, which is taken into account).  May take a considerable amount of time to complete.  If the current buffer is not in 7z-revisions mode, then it will execute 7z-revisions on that buffer before proceeding."
  (interactive)
  (let (region_beginning region_end  linenum_end  7zr-goto-rev-region-last-changed_cmp 7zr-goto-rev-region-last-changed_txt 7zr-goto-rev-region-last-changed_no_change 7zr-lines_not_changed 7zr-continue_searching (7zr-view_highlight_saved_state 7zr-view_highlightp) tmppoint )
	...
	(progn
	  ...
	  (when (> (point) tmppoint)
	    (exchange-point-and-mark))
	  (setq region_beginning (region-beginning))
	  (setq region_end (region-end))
	  
	  (setq 7zr-goto-rev-region-last-changed_linenum_begin (7zr-line-number-at-pos region_beginning))
	  (setq linenum_end (7zr-line-number-at-pos region_end))
	  (setq 7zr-goto-rev-region-last-changed_lines (- linenum_end 7zr-goto-rev-region-last-changed_linenum_begin))
	  )
      )
    (setq 7zr-view_highlightp nil)
 ;   (message (buffer-name (current-buffer)))
  ;; first making sure that point is in a 7zr-summary-view-revision (7zrv) mode buffer
    (cond
     
     ((and
       (bound-and-true-p 7zr-window)
       (string= 7zr-window "7zr-summary-view-revision")
;     (7zr-string-starts-with-p (buffer-name (current-buffer)) "rev")
 ;    (setq 7zr-goto-rev-region-last-changed_of_pos (string-match-p (buffer-name (current-buffer)) "_of_"))
					;    (setq 7zr-goto-rev-region-last-changed_original-version (substring (buffer-name (current-buffer)) 7zr-goto-rev-region-last-changed_of_pos -1))
       )
      (setq 7zr-goto-rev-region-last-changed_txt (7zr-goto-rev-region-last-changed_get_initial_text 7zr-goto-rev-region-last-changed_linenum_begin 7zr-goto-rev-region-last-changed_lines))
    ;(7zr-save-last-line-column)
    ;;(kill-buffer)
   ;; (switch-to-buffer 7zr-revisions_tmpbuffer)
   ...
   ;; 7zr-active-document_7zr-revisions_original_buffer
   ;; if we arent viewing a revision but there is already a 7z-revisions-summary-view opened somewhere on this document, then go to that
     ((and 7zr-buffer-misc
	   (buffer-live-p (get-buffer 7zr-buffer-misc)))
      (setq 7zr-buffer-misc_name (file-name-nondirectory (buffer-name (current-buffer))))  
      (setq 7zr-buffer-misc (concat 7zr-revisions_tmpbuffer-default_prefix_name 7zr-buffer-misc_name ))
      (setq 7zr-goto-rev-region-last-changed_txt (7zr-goto-rev-region-last-changed_get_initial_text 7zr-goto-rev-region-last-changed_linenum_begin 7zr-goto-rev-region-last-changed_lines))
...
;;;;;;  else start a new 7z-revision 
    ...
    (setq 7zr-goto-rev-region-last-changed_no_change nil)
    (save-window-excursion

      (setq 7zr-lines_not_changed t)
      (setq 7zr-continue_searching t)
      (message "This might take a while...")
      (while 7zr-continue_searching
	(setq 7zr-goto-rev-region-last-changed_cmp (7zr-goto-rev-region-last-changed_get_cmp_text 7zr-goto-rev-region-last-changed_lines))

	(if (not (string= 7zr-goto-rev-region-last-changed_cmp 7zr-goto-rev-region-last-changed_txt))
	    (progn 
	      
	      (setq 7zr-lines_not_changed nil)
	      (setq 7zr-continue_searching nil)
	      (when (bound-and-true-p 7zr-current-rev)
		(setq 7zr-continue_searching nil)
		(message (concat "lines change on rev " 7zr-current-rev))
		(setq 7zr-goto-rev-region-last-changed_linenum_begin (7zr-line-number-at-point))
		)
	      (when (bound-and-true-p 7zr-original-page)
	 
		(message "lines changed on original version 0 archived document.")
		)
	      (recenter-top-bottom)  
	      )
					; else	
	  (when (bound-and-true-p 7zr-original-page)
	    (setq 7zr-continue_searching nil)
	    (message "No changes in region.")
	    (setq 7zr-goto-rev-region-last-changed_no_change t) 
	    (7zr-view-quit)
	    (7zr-summary-quit)
	    )
	  )
      
    (when 7zr-continue_searching
	  (7zr-view_previous_page))
	)  ; while
      (setq 7zr-view_highlightp 7zr-view_highlight_saved_state )  
      )  ; save window excursion
    )
  )

(defun 7zr-reconstruct_refresh-some-pointers (rev linenum)
  (setq 7zr-summary-lastlinenum-at-point 7zr-summary-linenum-at-point)
  (setq 7zr-summary-linenum-at-point linenum)
  (setq 7zr-summary-lastrev-at-point 7zr-summary-rev-at-point)
  (setq 7zr-summary-rev-at-point rev)
...
(defun 7zr-reconstruct-rev-from-patches_advance_lastview_pointers ( current )
  ...
  (setq 7zr-summary-last-line-last 7zr-summary-last-line)
  )
...
(defun 7zr-reconstruct-rev-from-patches_MSWINDOWS (rev)
 ...
 pressed.   It calls an elisp implementation of the patch shell command."
  ...
  (7zr-reconstruct_refresh-some-pointers rev (7zr-line-number-at-point))

  (let ((rev_num (string-to-number rev)) saved_directory (rev_iter_string 7zr-pointer-lastviewed) (rev_iter_string_last "") shell_msg wip_buffer (wip_buffer_name (concat "rev" rev "_of_" 7zr-original-version)) diff_buffer (summary_buffer (current-buffer)) (point_halfway (/ (point-max) 2)))
    
;(set-buffer (get-buffer "7z-revisions")
...
;    (set-buffer 7zr-revisions_tmpbuffer)  ; this should instead be a buffer-local variable
    ...
    (setq 7zr-summary-current-line (7zr-line-number-at-point))
...
;;	(unless (string= 7zr-pointer-lastviewed "")
	  (if (7zr-revisions-validate-rev-p 7zr-pointer-lastviewed) ; 7zr-reconstruct-direction 7zr-pointer-lastviewed_nearest_ancestor)
		...
		(cond
		 ((= (1+ 7zr-summary-linenum-at-point) 7zr-summary-last-line)

		  (setq 7zr-reconstruct-direction -1)
		  (setq 7zr-reconstruct-dtag " -R -t "))
		 
		 ((= 7zr-summary-linenum-at-point (1+ 7zr-summary-last-line)) 
		  (setq 7zr-reconstruct-direction 1)
		  (setq 7zr-reconstruct-dtag " -t "))
...
;;	  ) ; unless no last viewed end
	...
	(if (not 7zr-valid-rev-in-tempp)
	    (progn
	      (if (< (7zr-line-number-at-point) (/ 7zr-revisions_tmpbuffer_lines 2))
		...
		(goto-char (point-max))
		(beginning-of-line)
		(previous-line) ; the latest revision is actually just another name for the last rev number
		(beginning-of-line)
		(looking-at "\\([0-9]+\\.[0-9]*\\)")
		(setq 7zr-pointer-lastviewed (match-string-no-properties 1))
		(setq rev_iter_string 7zr-pointer-lastviewed)
;	      (setq rev_iter_string_last 7zr-pointer-lastviewed_nearest_progeny)

		(setq rev_iter_string_last (number-to-string 7zr-patch-number))
		(set (make-local-variable '7zr-active-document_pointer_lastviewed) 7zr-pointer-lastviewed)
		(setq 7zr-reconstruct-direction -1)
		(setq 7zr-reconstruct-dtag " -R -t ")
    ...
    ; else  we are using a previous rev thats still in the temp directory and already set up
	  ...
	  (progn
;; 	      (when (= (7zr-line-number-at-point) 2)  ; for some reason the first revision needed a special case, not sure why, debug
...
;	    (find-file (concat 7zr-temp-directory "rev" 7zr-pointer-lastviewed "_of_"  7zr-original-version))  ; debug remove this line
...
;	  (rename-buffer wip_buffer_name)
	  ...
	  (if (= 7zr-reconstruct-direction -1)
	      (progn
		(setq rev_iter_string_last 7zr-pointer-lastviewed_nearest_progeny)
		(when (= (7zr-line-number-at-point) (- 7zr-revisions_tmpbuffer_lines 2))
		  (setq rev_iter_string_last (number-to-string 7zr-patch-number))
		  )
		)
	    (setq rev_iter_string_last 7zr-pointer-lastviewed_nearest_ancestor)
	    )
	  ...
	  (while  (not (string= rev_iter_string rev))   ; move the pointer
	    ...
	    (setq rev_iter_string (match-string-no-properties 1))        ;

	    (cond
	     ((= 7zr-reconstruct-direction -1)  ; up toward ancestors 
	     ...
	     ((= 7zr-reconstruct-direction 1) ; down toward progeny
...
;	    (set-buffer-file-coding-system 7zr-buffer_file_coding_system t t)
	    ...
	    (7z-revisions-mode 0)
	    (set-buffer-modified-p nil)
;;	    (setq 7zr-view-last-line (car (7zr-new-linenum-after-diff 7zr-view-last-line (concat 7zr-temp-directory 7zr-reconstruct-patch-to-apply) 7zr-reconstruct-dtag)))  ;; this is handled in 7zr-apply-patch
...
;	  (write-file (concat 7zr-temp-directory "rev" 7zr-pointer-lastviewed "_of_" 7zr-original-version))
	...
	(when 7zr-track-md5sum-hashes-p
	  ...
	  (setq 7zr-file-rev_hash (sha1 (current-buffer)))
	  
	  (if (not (string= 7zr-file-rev_hash 7zr-reconstruct-rev_hash))
	      (error "revision hash doesnt match! " 7zr-file-rev_hash)
	    )
...
;    (cd saved_directory)
    (set (make-local-variable '7zr-active-document) 7zr-buffer)
    (set (make-local-variable '7zr-current-rev) rev)
    (7zr-set-some-buffer-local-vars "7zr-summary-view-revision" (get-buffer 7zr-revisions_tmpbuffer))
...
(defun 7zr-reconstruct-rev-from-patches_LINUX (rev)
 ...
 pressed.  The difference between this function and the other of similar naming is that this one uses the `patch' shell command whereas the other uses an elisp variant of it."
  (interactive)
;  (pp rev)
;  (pp 7zr-summary-last-line)
  (setq 7zr-summary-linenum-at-point (7zr-line-number-at-point))
  (when (looking-at "\\([0-9]+\\.[0-9]+\\)")
    (when (not (string= rev (match-string-no-properties 1)))
      (error "rev isnt revision"))
    (setq rev (match-string-no-properties 1)))
  (setq 7zr-rev rev)
  ;; (when (and
  ;; 	 (= 7zr-summary-last-line 1)
  ;; 	 (= rev 7zr-patch-number))
    
  ;;   (setq 7zr-summary-last-line 2)
  ;;   )	 
;  (setq 7zr-pointer-lastviewed 7zr-lastviewed_saved)
...
;; 7zr-pointer-lastviewed_nearest_progeny
...
;    (set-buffer 7zr-revisions_tmpbuffer)  ; this should instead be a buffer-local variable
	...
	;(unless (string= 7zr-pointer-lastviewed "")
	  ...
	  (if (7zr-revisions-validate-rev-p 7zr-pointer-lastviewed) ; 7zr-reconstruct-direction 7zr-pointer-lastviewed_nearest_ancestor)
		...
		(setq 7zr-pointer-lastviewed_last 7zr-pointer-lastviewed)
;;		(rename-file (concat 7zr-temp-directory "rev" 7zr-pointer-lastviewed "_of_" 7zr-original-version) (concat 7zr-temp-directory 7zr-prepend-to-reconstruct_wip) t)
		(7zr-rename-file-from-temp-if-exists (concat  "rev" 7zr-pointer-lastviewed "_of_" 7zr-original-version) 7zr-prepend-to-reconstruct_wip)
...
;;;		     (< (- rev_num 7zr-pointer-lastviewed_num) (- 7zr-revisions_tmpbuffer_lines rev_num))		     
    ...
    ; if no last rev in temp or invalid then create last rev from original version or latest revision
...
;;		    (rename-file (concat 7zr-temp-directory 7zr-original-version) (concat 7zr-temp-directory 7zr-prepend-to-reconstruct_wip) t)
		    (7zr-rename-file-from-temp-if-exists 7zr-original-version 7zr-prepend-to-reconstruct_wip)
		    )
					; else
	       ; start at bottom of buffer and walk upwards
		(goto-char (point-max))
	...
	;;      (setq rev_iter_string_last 7zr-pointer-lastviewed_nearest_progeny)
		...
		(setq 7zr-reconstruct-dtag " -R -t ")
		(setq shell-msg (shell-command-to-string (concat "7z e -aoa -o" (7zr-shell-quote-argument 7zr-temp-directory) " " (7zr-shell-quote-argument (concat 7zr-archive-directory 7zr-archive-name)) " " (7zr-shell-quote-argument (concat 7zr-prepend-to-latest-revision  7zr-original-version)))))
		(7zr-rename-file-from-temp-if-exists (concat 7zr-prepend-to-latest-revision 7zr-original-version) 7zr-prepend-to-reconstruct_wip) 
;		(rename-file (concat 7zr-temp-directory 7zr-prepend-to-latest-revision 7zr-original-version) (concat 7zr-temp-directory 7zr-prepend-to-reconstruct_wip) t)
	  ...
	  ; deal with rej
	    ...
	    (if (file-exists-p (concat 7zr-temp-directory 7zr-prepend-to-reconstruct_wip ".rej"))
		  (if (eql 7zr-summary_discard_rejp t)
		   ...
		   ;; (7zr-rename-file-if-exists  (concat 7zr-temp-directory 7zr-prepend-to-reconstruct_wip ".rej") (concat 7zr-temp-directory 7zr-prepend-to-rej rev "_of_" 7zr-original-version ".rej"))
 		    (7zr-rename-file-from-temp-if-exists (concat  7zr-prepend-to-reconstruct_wip ".rej") (concat  7zr-prepend-to-rej rev "_of_" 7zr-original-version ".rej")) 
		   ;; (7zr-rename-file-if-exists  (concat 7zr-temp-directory 7zr-prepend-to-reconstruct_wip ".orig")  (concat 7zr-temp-directory 7zr-prepend-to-rej rev "_of_" 7zr-original-version ".orig"))
 		    (7zr-rename-file-from-temp-if-exists (concat 7zr-prepend-to-reconstruct_wip ".orig")  (concat 7zr-prepend-to-rej rev "_of_" 7zr-original-version ".orig"))
		    )
	      )
...
;;	(rename-file (concat 7zr-temp-directory 7zr-prepend-to-reconstruct_wip) (concat 7zr-temp-directory "rev" 7zr-pointer-lastviewed "_of_" 7zr-original-version) t)
	(7zr-rename-file-from-temp-if-exists (concat 7zr-prepend-to-reconstruct_wip) (concat "rev" 7zr-pointer-lastviewed "_of_" 7zr-original-version))
	
	(when 7zr-track-md5sum-hashes-p
       ;	    (with-current-buffer 7zr-active-document_buffer
	  (setq 7zr-reconstruct-rev_hash (gethash 7zr-pointer-lastviewed 7zr-hasht))
;	      )	    
	  (setq 7zr-file-rev_hash (7zr-string-trim-final-newline (7zr-string-trim-final-newline (shell-command-to-string (concat 7zr-sha1sum-command " " (7zr-shell-quote-argument (concat 7zr-temp-directory "rev" 7zr-pointer-lastviewed "_of_"  7zr-original-version)) 7zr-sha1sum-post-command (7zr-awk-cmd-string 1))))))    ;; comment out 
;;	  (setq 7zr-file-rev_hash (7zr-sha1-of-file-in-temp-directory (concat "rev" 7zr-pointer-lastviewed "_of_"  7zr-original-version)))  ;; comment out for speed 
	  (if (not (string= 7zr-file-rev_hash 7zr-reconstruct-rev_hash))
    ...
    (set (make-local-variable '7zr-active-document) 7zr-buffer)
    (set (make-local-variable '7zr-current-rev) rev)
    (7zr-set-some-buffer-local-vars "7zr-summary-view-revision"  7zr-revisions_lastbuffer)
;    (setf (buffer-local-value '7zr-archive-directory 7zr-revisions_lastbuffer) 'some-val)
;    (make-local-variable '7zr-archive-directory)
 ;   (setq 7zr-archive-directory (buffer-local-value '7zr-archive-directory 7zr-revisions_lastbuffer))
    (when 7zr-view_highlightp
...
(defun 7zr-summary-reconst-last-del ()
  ...
  (when (and
    ...
    (when  (not (string= 7zr-summary-reconst-last ""))
      ...
      (7zr-delete-file-if-exists (concat 7zr-temp-directory 7zr-reconstruct-patch-to-apply))
      (setq 7zr-summary-last-line (7zr-line-number-at-point))   ;debug
...
(setq 7zr-match-diff3-line "^====\\(.?\\)\C-m?$")
;;  "Pattern to match lines produced by diff3 that describe differences.")

(setq 7zr-match-diff-line
  (let ((x "\\([0-9]+\\)\\(\\|,\\([0-9]+\\)\\)"))
    (concat "^" x "\\([acd]\\)" x "\C-m?$")))
;;  "Pattern to match lines produced by diff that describe differences.")

(setq 7zr-match-diff-line2
  (let ((x "\\([0-9]+\\)\\(\\|,\\([0-9]+\\)\\)"))
    (concat "^" x "\\([acd]\\)" x "\C-m?$")))
;;  "Pattern to match lines produced by diff that describe differences.")

(setq 7zr-match-diff-standard-line
  (let ((x "\\([0-9]+\\)\\(\\|,\\([0-9]+\\)\\)"))
    (concat "^" x "\\([acd]\\)" x "\C-m?$")))
;;  "Pattern to match lines produced by diff that describe differences.")

(setq 7zr-match-diff-standard-hunk
  (let ((x "\\([0-9]+\\)\\(\\|,\\([0-9]+\\)\\)")
        (a "< \\(.*\\)")
	(b "> \\(.*\\)"))
...
(defun 7zr-what-is-relative-directory ( rel cur-full )
  (if (string= rel "")
      cur-full
    (let ( retval (saved_dir default-directory))
      (cd cur-full)
      (cd rel)
      (setq retval default-directory)
      (cd saved_dir)
      retval
      )
    )
...
(defun 7zr-parse-standard-diff-type ( parse-ancestorp )
  "This function returns a list containing the beginning point of every difference and is only used for highlighting changes when reviewing revisions.  This function calls '7zr-map-difference-ranges-to-point-ranges.  This function is called from '7zr-view-highlight-changes ."
  (let ((7zr-parse-standard-diff-type_diff_list '())
    (7zr-parse-standard-diff-type_lastbuffer (current-buffer))
    (7zr-parse-create-blank-file-tmpbuffer (generate-new-buffer-name "parse"))   ;; debugging
    
    (7zr-misc_save-directory-path default-directory)
	)
...
(defun 7zr-make-array ( lij  init_ele)
  "Functionally equivalent to make-vector, but can accept a list as the first argument specifying 2d or 3d matrix dimensions. Called from '7zr-longest-common-word-subsequence-difference-ranges "
  (if (listp lij)
      (if (eql (length lij) 2)	  
	  (progn
	    (let ( *i* *j* m-array)
...
(defun 7zr-longest-common-word-subsequence-difference-ranges (v1 v2 ancestorp )
  "Returns a list of ranges, the index of which starts at 0, inclusive, of consecutive word numbers that appear in v2 but do not appear in v1, given that ancestorp is true.  Otherwise returns range of word index positions appearing in v1 but not in v2.  This function is used for highlighting changes when reviewing revisions or when viewing diffs, given that the 7zr-view_highlightp variable is set to t.  ANCESTORP means that we are looking at the difference between the current revision and its nearest respective ancestor.  If its false, then it's the nearest respective progeny."
  (let* ( 
	 (l1 (length v1))
	 (l2 (length v2))
	 (p1_suf l1)
...
(defun 7zr-map-difference-ranges-to-point-ranges ( difference_ranges line_pos_v column_pos_v ancestorp)
  "This function returns a list of the beginning points of each range of differences which were provided by the difference_ranges list, and highlights the differences in the current-buffer.  This function is only used for highlighting changes when reviewing revisions.  If ancestorp is true then highlights in green otherwise red."
  (save-excursion
...
;		(untabify (point) (line-end-position))  ;; debug: to deal with tab characters in document that cause the program to crash without error
		...
		(if (<= column_pos_current line_end_position_col)
		    (progn    
		      ...
		      (move-to-column column_pos_current)
		      (setq begin (point))
...
;		(when ancestorp	(incf cntr)(when (= cntr 11)(debug)))  ;debug11
	 ...
	 ; highlight region
			    (if ancestorp
				(put-text-property begin end 'font-lock-face '(:background "green" :foreground "black"))
					; else
			      (if (equal (get-text-property begin 'font-lock-face) '(:background "green" :foreground "black"))
				  (put-text-property begin end 'font-lock-face '(:foreground "red" :background "green"))
				              ; else
				(put-text-property begin end 'font-lock-face '(:background "red" :background "black"))
				)
			      )
...
(defun 7zr-add-to-hashfile ( string_to_add )
  "This function is not being called by anything"
  (save-window-excursion
...
(defun 7zr-forward-patch-in-temp-directory ( file patch)
  "This program is called from 7zr-regenerate-all-hashes-from-diffs and leads to the calling of 7zr-apply-patch on MSwindows or 7zr-forward-patch-in-temp-directory_LINUX for linux, which perform the basic function of the linux patch command on unfied formatted diffs, in forward direction only."
  (if (or 
       (bound-and-true-p 7zr-active-document_buffer_is_msdos_p) 
       7zr-os_is_windows_p 
       ) 
      (7zr-forward-patch-in-temp-directory_WINDOWS file patch)
					; else
   ;; (7zr-forward-patch-in-temp-directory_WINDOWS file patch)
    (7zr-forward-patch-in-temp-directory_LINUX file patch)
    )
  )

(defun 7zr-forward-patch-in-temp-directory_WINDOWS ( file patch)
  (let ((saved_buffer (current-buffer)) (saved_dir default-directory) hashvalue file_buffer diff_buffer hash)
    (cd 7zr-temp-directory)
    (setq file_buffer (find-file file))
    (7z-revisions-mode 0)
    (when (bound-and-true-p undo-tree-mode)
      (undo-tree-mode 0)) 
    (setq diff_buffer (find-file patch))
    (set-buffer diff_buffer)
    (7z-revisions-mode 0)
    (when (bound-and-true-p undo-tree-mode)
      (undo-tree-mode 0)) 
    (7zr-apply-patch file 0 1)
    (set-buffer diff_buffer)
    (set-buffer-modified-p nil)
    (set-buffer file_buffer)
    (kill-buffer diff_buffer)
    (set-buffer file_buffer)
    (setq hash (sha1 file_buffer))
    (save-buffer file_buffer)
    (kill-buffer file_buffer)
    (cd saved_dir)
    (set-buffer saved_buffer)
    hash
    )
  )

    
(defun 7zr-forward-patch-in-temp-directory_LINUX ( file patch)
  (let ((saved-dir default-directory) hashvalue)
    (cd 7zr-temp-directory)
    (shell-command (concat 7zr-patch-command " " (7zr-shell-quote-argument file) " -i " (7zr-shell-quote-argument patch)))
    (setq hashvalue (7zr-string-trim-final-newline (7zr-string-trim-final-newline (shell-command-to-string (concat 7zr-sha1sum-command " " (7zr-shell-quote-argument file) 7zr-sha1sum-post-command (7zr-awk-cmd-string 1))))))    ;; comment out 
;;	  (setq hashvalue (7zr-sha1-of-file-in-temp-directory file ))  ;; comment out for speed 
    hashvalue
    )
  )
...
(defun 7zr-archive-save-metadata-file ( &optional cancel)
...
(defun 7zr-metafile-cancel ()
  (interactive)
  (7zr-archive-save-metadata-file t)
  )
...
(defun 7zr-populate-initial-global-vars_dired ()
  (let (tag_in_text)
    ;; extract dired-get-file-for-visit
    (setq 7zr-buffer_file_coding_system (symbol-name buffer-file-coding-system))
    (setq 7zr-buffer_is_msdos_p (string-match-p "dos" (symbol-name buffer-file-coding-system)))
   ...
   ; (setq 7zr-buffer (file-name-nondirectory 7zr-buffer-filename_including_dir))
    ...
    (set (make-local-variable '7zr-active-document_7zr-revisions_original_buffer) (current-buffer))
    (7zr-set-archive-directory-full 7zr-archive-directory)
...
(defun 7zr-populate-initial-global-vars_dired_regen ()
  (let (tag_in_text)
    ;; extract dired-get-file-for-visit
    (setq 7zr-buffer_file_coding_system (symbol-name buffer-file-coding-system))
    (setq 7zr-buffer_is_msdos_p (string-match-p "dos" (symbol-name buffer-file-coding-system)))
...
;; 7ZR-CLEAR-ALL-VARIABLES BEGINS


;; 7ZR-CLEAR_ALL_VARIABLES ENDS
...
(defun 7zr-refresh-all-bookmarks ()
   "For every bookmark in the bookmark-default-file that points to a file that has an associated 7z-revisions.el .7z archive, update the bookmark to point to the correct position indicated by its associated context.  This becomes useful as the script file grows.   For this script to work accurately, all opened buffers that are pointed to by bookmarks must first be saved.  But the script wont save the buffer first; the user must do it.  Also, this function saves the bookmark file after bookmarks have been updated via the bookmark-save command."
  ...
  ;; whether the new pointer matches the specified context.
   ...
   (interactive)
   (let (file-already-opened saved-point (last_buffer (current-buffer)) bkmrk_ptr_name bkmrk_ptr_file bkmrk_ptr_front-context bkmrk_ptr_rear-context bkmrk_ptr_pos bkmrk_ptr_file_dir bkmrk_ptr_file_reldir bkmrk_ptr_file_archive continue_looking bkmrk_ptr_pos_found bkmrk_ptr_pos_found bkmrk_ptr_linenum_found bkmrk_ptr_pos bkmrk_ptr_column_found  bookmark_buffer)
     (save-excursion
       (save-window-excursion
	 ...
	 (while (re-search-forward "^(?(\"\\(.*\\)\"\n (filename . \"\\(.*\\)\")\n (front-context-string . \"\\(.*\\)\")\n (rear-context-string . \"\\(.*\\)\")\n (position . \\([0-9]+\\)))" nil t)
	   ...
	   (setq bkmrk_ptr_file_dir (file-name-directory bkmrk_ptr_file))
	   (setq bkmrk_ptr_file_reldir (7zr-what-is-relative-directory 7zr-archive-directory_default bkmrk_ptr_file_dir))
	   (setq bkmrk_ptr_file_archive (concat bkmrk_ptr_file_reldir (file-name-nondirectory bkmrk_ptr_file) 7zr-archive-extension))
	   (save-match-data
	     (save-window-excursion
               (when (and
		      (file-exists-p bkmrk_ptr_file_archive)
		      (eql (string-match-p "No files to process" (shell-command-to-string (concat "7z e -aoa -o" (7zr-shell-quote-argument 7zr-temp-directory) " "  (7zr-shell-quote-argument bkmrk_ptr_file_archive)  " " (7zr-shell-quote-argument 7zr-archive-created-by-message)))) nil)
		      )
		 
	       
		 (message (concat "Bookmark " bkmrk_ptr_name " archive exists"))
		 (if (setq file-already-opened (get-file-buffer bkmrk_ptr_file))
		     (set-buffer file-already-opened)
					; else
		   (find-file-read-only bkmrk_ptr_file)
		   )
		 (goto-char bkmrk_ptr_pos)
		 (if (and
		      (looking-at bkmrk_ptr_front-context)
		      (looking-back bkmrk_ptr_rear-context))
		     
		     (message (concat "Bookmark " bkmrk_ptr_name " is in sync"))
					; else
		   (7z-revisions)
		   ...
		   (goto-char bkmrk_ptr_pos)
		   (setq continue_looking t)
		   (setq bkmrk_ptr_pos_found nil)
		   (setq bkmrk_ptr_linenum_found nil)
		   ...
		   (while continue_looking
		     ...
		     (if (and
			  ...
			  (looking-back bkmrk_ptr_rear-context))
			 (progn
			   (message (concat "Bookmark position in " 7zr-archive-extension " archive for " bkmrk_ptr_name " -> found it!"))
			   (setq continue_looking nil)
			   (setq bkmrk_ptr_pos_found (point))
			   (setq bkmrk_ptr_linenum_found (line-number-at-pos))
			   (setq bkmrk_ptr_column_found (7zr-what-column-at-point))
					;(setq bkmrk_ptr_pos_found_rev rev)
			   )
					; else we haven't found it and we're already on the last page, well actually first
		       (if (bound-and-true-p 7zr-original-page)
			   (progn
			     (message (concat "Bookmark " bkmrk_ptr_name " not found in its " 7zr-archive-extension " archive!"))
			     (setq continue_looking nil)
			     (7zr-view-quit)
			     (7zr-summary-quit)
			     )
		   ...
		   (if bkmrk_ptr_pos_found
		       (progn
			 (while (not (bound-and-true-p 7zr-latest-page))
			   (7zr-view_next_page))
			 (setq saved-point (point))
			 ...
			 (bookmark-set bkmrk_ptr_name)
			 (message (concat "Bookmark " bkmrk_ptr_name " updated!"))
			 )
		     )
		   )
		 (when (not file-already-opened)
		   (kill-buffer))
		 ) ; when file exists
	       )
	     )
	   )
	 (kill-buffer bookmark_buffer)
	 (bookmark-save)
	 (message "Finished running 7zr-refresh-all-bookmarks.")
	 ) ; save-windows-excursion
       )
     )
   )
